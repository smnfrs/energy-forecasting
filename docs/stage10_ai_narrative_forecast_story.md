...
**Status:** implemented 2026-07-09. All of 10a-10f built and tested end-to-end against real production models and a real Groq call (see below). Not yet committed or deployed to CI — `GROQ_API_KEY` still needs to be added as a GitHub Actions secret, and someone needs to decide whether `deploy/story/forecast/data/*.json` (the weekly yearly-recap output) gets committed to git or generated by a scheduled workflow, since the daily Pages deploy job doesn't regenerate it (same open question applies to Stage 9's own `deploy/story/data/*.json`, which is also currently uncommitted).
